...
def calculateRating(username, movie, data, simMatrix):
...
def mainFunction(path=''):
	...
	f = codecs.open(path + 'u1.test', 'r', 'ascii')
	#ui = calcMovieAvg(data)
	cnt = 0
	dif = 0
	sum = 0
	for line in f:
		...
		movie = fields[1]
		rating = int(fields[2].strip().strip('"'))
		calculated = calculateRating(user, movie, data, simMatrix)		
